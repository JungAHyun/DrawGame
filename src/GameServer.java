...
public class GameServer {
    ...
    private void go () { 
	   try {
		   ServerSocket serverSock = new ServerSocket(8000);	// 서버소켓객체 생성. 포트번호 8000 사용

    	   while(true) {
    		   Socket clientSocket = serverSock.accept();		// 클라이언트로부터 접속 대기. 접속한 클라이언트와 통신할 수 있는 전용 클라이언트 소켓객체 생성

    		   // 클라이언트를 위한 입출력 스트림 및 스레드 생성
    		   Thread t = new Thread(new ClientHandler(clientSocket));
    		   t.start();									
    		   System.out.println("S : 클라이언트 연결 완료");		
    	   }
       } catch(Exception ex) {
		   System.out.println("S : 클라이언트  연결 중 이상발생");	
    	   ex.printStackTrace();
       }
    }
    ...
    private class ClientHandler implements Runnable {
    	...
    	public void run() {
    		...
    		try {
    			...
    			System.out.println("S : 클라이언트 접속 종료");																
    		}
    	} 
    } 
